...
def compute_relative_rotation_error_rpy(gt_rotation: np.ndarray, est_rotation: np.ndarray):
    ...
    return np.absolute(gt_x-est_x),np.absolute(gt_y-est_y),np.absolute(gt_z-est_z)
...
#sch
from geotransformer.modules.ops import pairwise_distance_np
def evaluate_overlap(
    ...
    # dsit_mat = np.sqrt(pairwise_distance_np(ref_node, src_node))

    # src_dst_min_dist, _ = np.min(dsit_mat, dim=1)  # BxM
    # dst_src_min_dist, _ = np.min(dsit_mat, dim=0)  # BxN

    # src_gt = np.zeros_like(src_dst_min_dist)
    # src_gt[src_dst_min_dist<overlap_thres] = 1
    # ref_gt = np.zeros_like(dst_src_min_dist)
    # ref_gt[dst_src_min_dist<overlap_thres] = 1

    # n2n_pos = ((ref_n2n_scores_c*ref_gt).mean() + (src_n2n_scores_c*src_gt).mean()/2)
    # n2n_neg = ((ref_n2n_scores_c*(1-ref_gt)).mean() + (src_n2n_scores_c*(1-src_gt)).mean()/2)

    ###################
    # overlap loss
    dsit_mat = np.sqrt(pairwise_distance_np(ref_node, src_points_f))
    ref_src_n2p_min_dist = np.min(dsit_mat, axis=1)
    ref_gt = np.zeros_like(ref_src_n2p_min_dist)
    ref_gt[ref_src_n2p_min_dist<n2p_overlap_thres] = 1
    
    dsit_mat = np.sqrt(pairwise_distance_np(src_node, ref_points_f))
    src_ref_n2p_min_dist = np.min(dsit_mat, axis=1)
    src_gt = np.zeros_like(src_ref_n2p_min_dist)
    src_gt[src_ref_n2p_min_dist<n2p_overlap_thres] = 1

    ref_mask = ref_gt==1
    src_mask = src_gt==1
    n2p_p_mean = ((ref_n2p_scores_c[ref_mask]).mean() + (src_n2p_scores_c[src_mask]).mean())/2
    n2p_n_mean = ((ref_n2p_scores_c[~ref_mask]).mean() + (src_n2p_scores_c[~src_mask]).mean())/2
    n2p_p_std = ((ref_n2p_scores_c[ref_mask]).std() + (src_n2p_scores_c[src_mask]).std())/2
    n2p_n_std = ((ref_n2p_scores_c[~ref_mask]).std() + (src_n2p_scores_c[~src_mask]).std())/2

    return {
        'n2p_p_mean': n2p_p_mean,
        'n2p_n_mean': n2p_n_mean,
        'n2p_p_std': n2p_p_std,
        'n2p_n_std': n2p_n_std,
    }

def evaluate_node_overlap(ref_points, src_points, ref_corr_indices, src_corr_indices, gt_corr_indices, gt_corr_overlap):
    ...
    overlap_mat = np.zeros_like(gt_corr_mat)
    overlap_mat[ref_gt_corr_indices, src_gt_corr_indices] = gt_corr_overlap

    gt_max_overlap_for_each_node = (overlap_mat.max(0)[overlap_mat.max(0)>0].mean() + overlap_mat.max(1)[overlap_mat.max(1)>0].mean())/2


    pred_overlap = overlap_mat[ref_corr_indices, src_corr_indices].mean()
    gt_overlap = gt_corr_overlap.mean()
    pred_true_overlap = overlap_mat[gt_corr_mat*pred_corr_mat>0].mean()


    return pred_overlap, gt_overlap, pred_true_overlap, gt_max_overlap_for_each_node
