...
def get_node_correspondences(
    ...
    # pcd0 = to_o3d_pcd(src_knn_points.view(-1,3))
    # pcd1 = to_o3d_pcd(ref_knn_points.view(-1,3))      
    # o3d.visualization.draw_geometries([pcd0.paint_uniform_color([1, 0, 0])+pcd1.paint_uniform_color([0, 1, 0])])
                
    # pcd0 = to_o3d_pcd(src_nodes.view(-1,3))
    # pcd1 = to_o3d_pcd(ref_nodes.view(-1,3))      
    # o3d.visualization.draw_geometries([pcd0.paint_uniform_color([1, 0, 0])+pcd1.paint_uniform_color([0, 1, 0])])
    ...
    # if get_one_to_one_corr:
    #     # find one-to-one node correspondences
    #     corr_overlaps_mat = torch.zeros_like(intersect_mat,dtype=torch.float)
    #     corr_overlaps_mat[ref_corr_indices,src_corr_indices] = corr_overlaps
    #     ref_max_overlap = corr_overlaps_mat.max(-1)
    #     src_max_overlap = corr_overlaps_mat.max(-2)
    #     ref_corr_mask = ref_max_overlap.values > 0
    #     src_corr_mask = src_max_overlap.values > 0
    #     ref_idx = torch.arange(ref_nodes.shape[0]).to(corr_overlaps_mat.device)
    #     src_idx = torch.arange(src_nodes.shape[0]).to(corr_overlaps_mat.device)
    #     ref_1to1_corr_indices = torch.stack([ref_idx[ref_corr_mask], ref_max_overlap.indices[ref_corr_mask]],dim=1)
    #     src_1to1_corr_indices = torch.stack([src_idx[src_corr_mask], src_max_overlap.indices[src_corr_mask]],dim=1)

    #     return ref_1to1_corr_indices, src_1to1_corr_indices

    return corr_indices, corr_overlaps

@torch.no_grad()
def get_node_overlap(
    ref_nodes: torch.Tensor,
    src_nodes: torch.Tensor,
    ref_knn_points: torch.Tensor,
    src_knn_points: torch.Tensor,
...
# sch
@torch.no_grad()
def get_node_correspondences_disance(
    ref_nodes: torch.Tensor,
    src_nodes: torch.Tensor,
    transform: torch.Tensor,
    pos_radius: float,
    ...
    dist_mat = pairwise_distance(ref_nodes, src_nodes)  # (M, N)

    masks = torch.zeros_like(dist_mat, dtype=torch.bool)

    ref_idx = torch.arange(ref_nodes.shape[0]).to(ref_nodes.device)
    ref_src_min_dist, ref_src_min_indice = torch.min(dist_mat, dim=1, keepdim=False)  # BxM
    ref_src_mask = ref_src_min_dist<pos_radius
    masks[ref_idx[ref_src_mask], ref_src_min_indice[ref_src_mask]] = True

    src_idx = torch.arange(src_nodes.shape[0]).to(src_nodes.device)
    src_ref_min_dist, src_ref_min_indice = torch.min(dist_mat, dim=0, keepdim=False)  # BxM
    src_ref_mask = src_ref_min_dist<pos_radius
    masks[src_ref_min_indice[src_ref_mask], src_idx[src_ref_mask]] = True

    masks = torch.logical_and(masks, node_mask_mat) 
...
def get_correspondences(
    ...
    neighbor_indices = radius_search(q_nodes,s_nodes,q_nodes.shape[0],s_nodes.shape[0],radius,neighbor_limit)
