...
def load_kitti_gt_txt(txt_root, seq):
    ...
    with open(osp.join(txt_root, 'icp10', seq), 'r') as f:
...
def make_dataset_kitti(txt_path, mode):
    ...
    seq_list = ['kaist01','riveside01','sejong01']
    # seq_list = ['01']
...
class OdometrymulranPairDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(OdometrymulranPairDataset, self).__init__()

        self.mulran_root = mulran_root
        self.subset = subset
        ...
        self.metadata = make_dataset_kitti(self.mulran_root, subset)
    ...
    def __getitem__(self, index):
        ...
        ref_points = self._load_point_cloud(osp.join(self.mulran_root, 'downsampled_xyzi', data_dict['seq_id'], '%d.npy' % (data_dict['ref_frame'])))[:,:3]
        src_points = self._load_point_cloud(osp.join(self.mulran_root, 'downsampled_xyzi', data_dict['seq_id'], '%d.npy' % (data_dict['src_frame'])))[:,:3]
