...
class OdometryKittiPairDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.metadata = make_dataset_kitti(osp.join(self.dataset_root,'icp%i'%benchmark_distance),subset)
    ...
    def __getitem__(self, index):
        ...
        # src_frame = osp.join('/mnt/Mount/Dataset/KITTI_odometry/sequences','%02d'%data_dict['seq_id'], 'velodyne/%06d.bin' % (data_dict['src_frame']))
        # ref_points_raw = np.fromfile(ref_frame, dtype=np.float32).reshape(-1, 4)[:, :3]
        # src_points_raw = np.fromfile(src_frame, dtype=np.float32).reshape(-1, 4)[:, :3]
        # ref_points_raw = self._load_point_cloud(osp.join('/mnt/Mount/Dataset/KITTI_odometry/sequences','%02d'%data_dict['seq_id'], 'velodyne/%06d.bin' % (data_dict['ref_frame'])))
        # src_points_raw = self._load_point_cloud(osp.join('/mnt/Mount/Dataset/KITTI_odometry/sequences','%02d'%data_dict['seq_id'], 'velodyne/%06d.bin' % (data_dict['src_frame'])))
        ...
        # data_dict['ref_points_raw'] = ref_points_raw.astype(np.float32)
        # data_dict['src_points_raw'] = src_points_raw.astype(np.float32)
