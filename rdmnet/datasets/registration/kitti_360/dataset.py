...
def load_kitti_gt_txt(txt_root, seq):
    ...
    with open(osp.join(txt_root, 'icp10', '%04d'%seq), 'r') as f:
    # with open(osp.join(txt_root, 'icp10_for_odom', '%04d'%seq), 'r') as f:
        ...
        for i, line_str in enumerate(lines_list):
            ...
            # remove bad pair
            # bad_list = [[2, 4063, 4068], [2, 4069, 4088], [2, 5721, 5733], [4, 1, 86], [10, 853, 853]]
            # if [seq,anc_idx,pos_idx] in bad_list:
            #     continue
...
class OdometryKitti360PairDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.metadata = make_dataset_kitti(self.dataset_360_root, subset)
    ...
    def __getitem__(self, index):
        ...
        ref_points = self._load_point_cloud(osp.join(self.dataset_360_root, 'downsampled_xyzi','%04d'%data_dict['seq_id'], '%010d.npy' % (data_dict['ref_frame'])))[:,:3]
        src_points = self._load_point_cloud(osp.join(self.dataset_360_root, 'downsampled_xyzi','%04d'%data_dict['seq_id'], '%010d.npy' % (data_dict['src_frame'])))[:,:3]
