...
def load_kitti_gt_txt(txt_root, seq):
    ...
    with open(osp.join(txt_root, 'icp10','%02d'%seq), 'r') as f:
    # with open(osp.join(txt_root, 'raw10_for_odom','%02d'%seq), 'r') as f:
...
class OdometryApolloPairDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        dataset_root,
        subset,
        ...
        super(OdometryApolloPairDataset, self).__init__()
        ...
        self.metadata = make_dataset_kitti(self.dataset_root,subset)
