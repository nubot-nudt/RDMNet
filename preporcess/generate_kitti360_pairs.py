...
class SparseDataset(Dataset):
    ...
    def prepare_kitti360_pairs(self):
        ...
        for seq in self.DATA_FILES:
            # fnames = glob.glob(self.root_path + '/data_3d_raw/2013_05_28_drive_%04d_sync/velodyne_points/data/*.bin' % seq)
            # assert len(fnames) > 0, f"Make sure that the path {self.root_path} has data {seq}"
            ...
            curr_time = inames[0]
            f = open(os.path.join(self.root_path,'icp10','%02d'% seq),'a')
            while curr_time in inames:
                ...
                if next_time in inames:
                    ...
                    # T_cam0_velo = torch.tensor(cam2velo, dtype=torch.float, device='cpu')
                    # T_gt = torch.einsum('ab,bc,cd,de->ae', T_cam0_velo, torch.inverse(pose2), pose1, torch.inverse(T_cam0_velo))
                    ...
                    # o3d.visualization.draw_geometries([pcd0.paint_uniform_color([1, 0, 0])+pcd1.paint_uniform_color([0, 1, 0])+to_o3d_pcd(xyz0_tensor_t).translate([0,50,0]).paint_uniform_color([1, 0, 0])+pcd1.translate([0,50,0]).paint_uniform_color([0, 1, 0])])
